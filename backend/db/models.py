...
class User(Base):
    ...
    is_active = Column(
        Boolean, nullable=False, server_default="true", comment="Whether user account is active"
    )
    email_verified = Column(
        Boolean, nullable=False, server_default="false", comment="Whether email has been verified"
    )
    last_login = Column(TIMESTAMP, nullable=True, comment="Timestamp of last successful login")
    onboarded = Column(
        Boolean,
        nullable=False,
    ...
    preferred_categories = Column(
        JSONB, nullable=False, server_default="[]", comment="List of preferred category IDs"
    )
    style_preferences = Column(
        JSONB, nullable=False, server_default="{}", comment="Style tags and preferences"
    )

    # Stats
    total_interactions = Column(
        Integer, nullable=False, server_default="0", comment="Total number of user interactions"
    )
...
class UserInteraction(Base):
    ...
    # Session and context
    session_id = Column(
        String(128), nullable=True, index=True, comment="Session ID for grouping interactions"
    )
    context = Column(
        String(64),
        nullable=True,
    ...
    # Processing flags
    processed_for_embedding = Column(
        Boolean,
        nullable=False,
        server_default="false",
    ...
    processed_at = Column(
        TIMESTAMP, nullable=True, comment="When this interaction was processed for embeddings"
    )
...
class Product(Base):
    ...
    # Relationships
    interactions = relationship(
        "UserInteraction", back_populates="product", cascade="all, delete-orphan"
    )
...
class ProductEmbedding(Base):
    ...
    embedding_type = Column(
        String(50), nullable=False, index=True, comment="Type: text, image, multimodal"
    )

    # Legacy embedding (ARRAY format)
    embedding = Column(ARRAY(Float, dimensions=1), nullable=True, comment="Legacy embedding array")
...
class TaskExecution(Base):
    ...
    id = Column(PGUUID(as_uuid=True), primary_key=True, server_default=text("uuid_generate_v4()"))
    task_id = Column(
        String(255), unique=True, index=True, nullable=False, comment="Celery task ID (UUID)"
    )
    task_name = Column(
        String(255),
        nullable=False,
    ...
    progress_percent = Column(Integer, nullable=True, comment="Task progress percentage (0-100)")
    progress_current = Column(Integer, nullable=True, comment="Current item being processed")
    progress_total = Column(Integer, nullable=True, comment="Total items to process")
    progress_message = Column(Text, nullable=True, comment="Human-readable progress message")

    # Task metadata
    args = Column(JSONB, nullable=True, comment="Task positional arguments")
    kwargs = Column(JSONB, nullable=True, comment="Task keyword arguments")
    task_metadata = Column(JSONB, nullable=True, comment="Additional task metadata")

    # Execution details
    worker_name = Column(String(255), nullable=True, comment="Celery worker that executed the task")
    queue_name = Column(String(100), nullable=True, comment="Queue the task was sent to")
    retries = Column(
        Integer, nullable=False, server_default="0", comment="Number of retry attempts"
    )
    max_retries = Column(Integer, nullable=True, comment="Maximum number of retries allowed")
    ...
    started_at = Column(TIMESTAMP, nullable=True, comment="When task started executing")
    completed_at = Column(
        TIMESTAMP, nullable=True, comment="When task finished (success or failure)"
    )

    # Results and errors
    result = Column(JSONB, nullable=True, comment="Task result data")
    error = Column(Text, nullable=True, comment="Error message if task failed")
    traceback = Column(Text, nullable=True, comment="Full error traceback")
