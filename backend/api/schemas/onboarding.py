...
class OnboardingCompleteRequest(BaseModel):
    ...
    selected_product_ids: List[str] = Field(
        ..., min_length=3, max_length=5, description="3-5 product IDs selected from moodboard"
    )
...
class OnboardingCompleteResponse(BaseModel):
    ...
    embedding_metadata: Optional[Dict[str, Any]] = Field(
        None, description="Information about the created embedding (confidence, method, etc.)"
    )
